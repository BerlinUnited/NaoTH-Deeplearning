...
from models.yolov3 import bbox_iou
...
class Dataset(object):
    ...
    def __init__(self, dataset_type):
        ...
        my_config = config_dict["yolov4_tiny"]

        self.annot_path  = my_config["TRAIN_ANNOT_PATH"] if dataset_type == 'train' else my_config["TEST_ANNOT_PATH"]
        self.input_sizes = my_config["TRAIN_INPUT_SIZE"] if dataset_type == 'train' else my_config["TEST_INPUT_SIZE"]
        self.batch_size  = my_config["TRAIN_BATCH_SIZE"] if dataset_type == 'train' else my_config["TEST_BATCH_SIZE"]
        self.data_aug    = my_config["TRAIN_DATA_AUG"]   if dataset_type == 'train' else my_config["TEST_DATA_AUG"]

        self.train_yolo_tiny = my_config["TRAIN_YOLO_TINY"]
        self.train_input_sizes = my_config["TRAIN_INPUT_SIZE"]
        self.strides = np.array(YOLO_STRIDES)
        self.classes = read_class_names(my_config["TRAIN_CLASSES"])
        print(self.classes)
        
        self.num_classes = len(self.classes)
        self.anchors = (np.array(YOLO_ANCHORS).T/self.strides).T
        self.anchor_per_scale = my_config["YOLO_ANCHOR_PER_SCALE"]
        self.max_bbox_per_scale = my_config["YOLO_MAX_BBOX_PER_SCALE"]
    ...
    def load_annotations(self):
        ...
        # FIXME: figure out what the outcome should be. 
