...
def upload_track_annotation(task_id):
    ...
    with requests.Session() as session:
        ...
        try:
            response = session.patch(url, headers={"x-csrftoken": csrftoken, "Authorization": "token " + token}, json=annotation_dict)
...
def create_dummy_task_1():
    ...
    task_list = [
        {
            "path": "/repl/Experiments/cvat_test_images.zip",
            "name": "dummy_task_1"
        }
    ]
    with requests.Session() as session:
...
def create_dummy_task_2():
    ...
    task_list = [
        {
            "path": "/repl/Experiments/tracking_test_dataset.zip",
            "name": "dummy_task_2"
        }
    ]
    with requests.Session() as session:
