...
def predict_batch_ball(model, valid_generator, idx, obj_threshold=0.5, nms_threshold=0.1, draw=False, force=False, resize_factor=1, t=None):
    ...
    l = loss.ball_cnn_loss(y_true_batch, netouts).numpy()

    shape_0 = x_batch[0].shape[0]
    shape_1 = x_batch[0].shape[1]
    converted_netouts = [_sigmoid(netouts[..., 0]),
                         _sigmoid(netouts[..., 1]) * shape_0,
                         _sigmoid(netouts[..., 2]) * shape_1]

    batch_predictions = []
    batch_images = []
    for i in range(len(x_batch)):
        binary_y_pred = int(converted_netouts[0][i] > obj_threshold)
        binary_y_true = int(y_true_batch[i][0] >= FLAGS.iou_threshold)
        if binary_y_true == 1:
            deviation = np.abs(converted_netouts[1][i] - y_true_batch[i][1]) + np.abs(converted_netouts[2][i] - y_true_batch[i][2])
        else:
            deviation = 0.0
        ...
        batch_prediction.append(converted_netouts[0][i])    # 2 - y_pred
        ...
        batch_prediction.append(y_true_batch[i][5])         # 6 - blurriness
        batch_prediction.append(l[i])                       # 7 - loss
        ...
        if draw:
            ...
            if binary_y_true == 1:
                cv2.drawMarker(image, (int(round(y_true_batch[i][1] * resize_factor)), int(round(y_true_batch[i][2] * resize_factor))), (0, 0, 255), cv2.MARKER_CROSS, resize_factor * 2, max(2, resize_factor))
                cv2.circle(image, (int(round(y_true_batch[i][1] * resize_factor)), int(round(y_true_batch[i][2] * resize_factor))), int(round(y_true_batch[i][3] * resize_factor / 2.0)), (0, 0, 255), max(2, resize_factor))
