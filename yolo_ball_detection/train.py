...
def yolo_loss(y_true, y_pred):
    ...
    xy_loss    = tf.keras.backend.sum(tf.keras.backend.sum(tf.keras.backend.square(y_true_xy - y_pred_xy),axis=-1)*y_true_conf, axis=-1)
    wh_loss    = tf.keras.backend.sum(tf.keras.backend.sum(tf.keras.backend.square(tf.keras.backend.sqrt(y_true_wh) - tf.keras.backend.sqrt(y_pred_wh)), axis=-1)*y_true_conf, axis=-1)
    ...
    intersect_wh = tf.keras.backend.maximum(tf.keras.backend.zeros_like(y_pred_wh), (y_pred_wh + y_true_wh)/2 - tf.keras.backend.square(y_pred_xy - y_true_xy) )
...
def load_data():
    ...
    for image_path in images:
        ...
        label_file = Path("datasets/ball_only_dataset_80-60/labels/rqeruwggyqijgzvnxvimav") / Path(image_path.name).with_suffix(".txt")
