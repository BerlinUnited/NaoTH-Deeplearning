...
def bhuman_segmentation_lower_yuv():
    ...
    x7 = tf.keras.layers.Convolution2D(3, (3, 3), strides=(1, 1), padding="same", name="Conv2D_4", activation='sigmoid')(x7) # train only on robots for now

    classifier = tf.keras.Model(inputs=inputs, outputs=x7, name="bhuman-bop")
    return classifier

def bhuman_segmentation_lower_y():
...
def bhuman_segmentation_lower_gray():
    ...
    x7 = tf.keras.layers.Add()([x6, x6r])
    #x7 = tf.keras.layers.Convolution2D(4, (3, 3), strides=(1, 1), padding="same", name="Conv2D_4")(x7) # original bhuman but we only have 3 classes and without an activation function the values are all over the place
