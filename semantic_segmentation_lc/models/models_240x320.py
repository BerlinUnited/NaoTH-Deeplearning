...
def bhuman_segmentation_y_channel_240x320x1():
    ...
    x = tf.keras.layers.ReLU(name="activation_1")(x)
    x1 = tf.keras.layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding="same")(x)
...
def bhuman_segmentation_y_channel_240x320x1_modified_v1():
    inputs = tf.keras.Input(shape=(240, 320, 1))
    ...
    x = tf.keras.layers.ReLU(name="activation_1")(x)
    x1 = tf.keras.layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding="same")(x)
    # x = tf.keras.layers.SeparableConv2D(filters=16, kernel_size=(3,3), padding='same', strides=(1, 1), activation='linear')(x1)
    # x = tf.keras.layers.BatchNormalization(axis=3)(x)
    # x = tf.keras.layers.ReLU(name="activation_2")(x)
...
def bhuman_segmentation_y_channel_240x320x1_modified_v2():
    inputs = tf.keras.Input(shape=(240, 320, 1))
    ...
    # x = tf.keras.layers.ReLU(name="activation_5")(x)
    # x3 = tf.keras.layers.Add()([x1, x])
    x3l = tf.keras.layers.Convolution2D(24, (1, 1), strides=(2, 2), padding="valid", name="Conv2D_2")(x1)
    x3l = tf.keras.layers.BatchNormalization(axis=3)(x3l)
