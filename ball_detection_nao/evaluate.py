...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to the image database containing test data." "Default is imgdb.pkl in the data folder.",
    )
    parser.add_argument(
        "-m",
        "--model-path",
    ...
    if target_input_shape != input_shape:
        ...
        for image in x:
            temp = cv2.resize(
                image, dsize=(target_input_shape[0], target_input_shape[1]), interpolation=cv2.INTER_CUBIC
            )
