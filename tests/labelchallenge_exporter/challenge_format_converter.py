...
def clipped():
    xml_label_file = "annotations.xml"

    with open('naoth_annotations_clipped.csv', 'w') as f:
        ...
        for image in annotation_root:
            ...
            for bbox in image:
                ...
                # TODO handle case of bounding box complety outside
                if float(xtl) < 0: xtl = 0
                if float(ytl) < 0: ytl = 0
                if float(xbr) >= 1920: xbr = 1919
                if float(ybr) >= 1080: ybr = 1080
