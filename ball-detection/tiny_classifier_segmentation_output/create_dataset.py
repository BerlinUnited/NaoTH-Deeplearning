...
from helper import get_postgres_cursor, get_minio_client, get_labelstudio_client, download_from_minio, load_image_as_yuv422_original
...
def download_patches():
    ...
    for bucket_name in sorted(data):
        ...
        file_path = download_from_minio(client=mclient, bucket_name=bucket_name, filename="patches_seg.zip", output_folder="downloads")
        print(f"\t{file_path}")
        with ZipFile(file_path, 'r') as f:
            f.extractall("datasets_segmentation")
        
        file_path = download_from_minio(client=mclient, bucket_name=bucket_name, filename="patches.zip", output_folder="downloads")
        print(f"\t{file_path}")
        with ZipFile(file_path, 'r') as f:
