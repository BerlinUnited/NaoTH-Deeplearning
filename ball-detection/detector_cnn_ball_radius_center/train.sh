...
while getopts e:s:f:i:n:b:d:t:v: flag
do
    case "${flag}" in
...
if [ -z "$data_root" ] || [ -z "$data_train" ] || [ -z "$data_val" ]; then
    echo "Error: data_root (-d), data_train (-t) and data_val (-v) parameters are required."
    exit 1
fi
